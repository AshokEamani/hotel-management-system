...
@app.route('/view_amenities/<string:id>/')
def view_features(id):
    cur = mysql.connection.cursor()
    ...
    return render_template('view_amenities.html', amenities=amenities)
...
class RegisterForm(Form):
    ...
    password = PasswordField('Password', [validators.DataRequired(),validators.Length(min=6, max=150), validators.EqualTo('confirm', message='Passwords do not match')])
...
def edit_amenities(id):
    ...
    form.type.data = str(amenities['a_type'])
    form.status.data = str(amenities['a_status'])
    form.capacity.data = str(amenities['a_capacity'])
    form.title.data = str(amenities['a_title'])
    form.description.data  = amenities['a_description']
    ...
    if form.validate_on_submit():
        ...
        cur.execute("UPDATE features SET a_type=%s, a_status=%s, a_capacity=%s, a_title=%s, a_description=%s WHERE a_id=%s", (type, status, capacity, title, description, id))
    ...
    print(form.errors)
    return render_template('edit_features.html', form=form)

@app.route('/delete_amenities/<string:id>', methods=['GET', 'POST'])
@is_logged_in
def delete_amenities(id):
    cur=mysql.connection.cursor()

    cur.execute("DELETE FROM features WHERE a_id=%s", [id])
